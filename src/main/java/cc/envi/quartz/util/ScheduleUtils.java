...
public class ScheduleUtils {
    ...
    public static void createScheduleJob(Scheduler scheduler, TaskEntity scheduleJob) {
        try {
            // 构建job信息
            JobDetail jobDetail = JobBuilder.newJob(ScheduleJob.class).withIdentity(getJobKey(scheduleJob.getTaskId()))
                    .build();
            ...
            CronTrigger trigger = TriggerBuilder.newTrigger().withIdentity(getTriggerKey(scheduleJob.getTaskId()))
            ...
            if (scheduleJob.getStatus().equals(TaskEntity.ScheduleStatus.PAUSE.getValue())) {
                pauseJob(scheduler, scheduleJob.getTaskId());
            }
        } catch (SchedulerException e) {
    ...
    public static void updateScheduleJob(Scheduler scheduler, TaskEntity scheduleJob) {
        try {
            TriggerKey triggerKey = getTriggerKey(scheduleJob.getTaskId());

            // 表达式调度构建器
            CronScheduleBuilder scheduleBuilder = CronScheduleBuilder.cronSchedule(scheduleJob.getCronExpression())
                    .withMisfireHandlingInstructionDoNothing();

            CronTrigger trigger = getCronTrigger(scheduler, scheduleJob.getTaskId());
    ...
    public static void run(Scheduler scheduler, TaskEntity scheduleJob) {
        try {
            ...
            scheduler.triggerJob(getJobKey(scheduleJob.getTaskId()), dataMap);
        } catch (SchedulerException e) {
